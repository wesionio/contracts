...
contract BatchTransferEtherAndwesion is Ownable{
    ...
    IERC20 wesion = IERC20(0x82070415FEe803f94Ce5617Be1878503e58F0a6a);

    /**
     * @dev Batch transfer both.
     */
    function batchTransfer(address payable[] memory accounts, uint256 etherValue, uint256 wesionValue) public payable {
        uint256 __etherBalance = address(this).balance;
        uint256 __wesionAllowance = wesion.allowance(msg.sender, address(this));

        require(__etherBalance >= etherValue.mul(accounts.length));
        require(__wesionAllowance >= wesionValue.mul(accounts.length));
